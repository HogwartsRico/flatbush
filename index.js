...
class Flatbush {
    constructor(numItems, nodeSize, ArrayType) {
    ...
    finish() {
        ...
        for (let i = 0; i < this._numItems; i++) {
            const x = Math.floor(hilbertMax * (this.data[5 * i + 1] - this._minX) / width);
            const y = Math.floor(hilbertMax * (this.data[5 * i + 2] - this._minY) / height);
        ...
        do {
            ...
            while (pos < end) {
                ...
                for (let i = 0; i < this._nodeSize && pos < end; i++) {
                    pos++; // skip index
    ...
    search(minX, minY, maxX, maxY, filterFn) {
        ...
        while (nodeIndex !== undefined) {
            ...
            const end = upperBound(nodeIndex, this._levelBoundaries);
            ...
            for (let i = 0; i < this._nodeSize; i++) {
                ...
                if (nodeIndex < this._numItems * 5) {
                    // leaf item
                    const add = filterFn !== undefined ? filterFn(index) : true;
